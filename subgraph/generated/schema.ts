...
export class Dex223DayData extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save Dex223DayData entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type Dex223DayData must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`,
      );
      store.set("Dex223DayData", id.toString(), this);
    }
  }

  static loadInBlock(id: string): Dex223DayData | null {
    return changetype<Dex223DayData | null>(
      store.get_in_block("Dex223DayData", id),
    );
  }

  static load(id: string): Dex223DayData | null {
    return changetype<Dex223DayData | null>(store.get("Dex223DayData", id));
  }
...
export class TokenConvertible extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save TokenConvertible entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type TokenConvertible must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`,
      );
      store.set("TokenConvertible", id.toString(), this);
    }
  }

  static loadInBlock(id: string): TokenConvertible | null {
    return changetype<TokenConvertible | null>(
      store.get_in_block("TokenConvertible", id),
    );
  }

  static load(id: string): TokenConvertible | null {
    return changetype<TokenConvertible | null>(
      store.get("TokenConvertible", id),
    );
  }

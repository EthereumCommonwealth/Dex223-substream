...
export function handleCollectHelper(
  ...
  const collect = new Collect(transaction.id + "-" + tx.logOrdinal.toString());
  collect.transaction = transaction.id;
  collect.timestamp = BigInt.fromI32(tx.timestamp as i32);
  collect.pool = pool.id;
  collect.owner = Address.fromString(event.owner);
  collect.amount0 = collectedAmountToken0;
  collect.amount1 = collectedAmountToken1;
  collect.amountUSD = trackedCollectedAmountUSD;
  collect.tickLower = BigInt.fromI32(event.tickLower);
  collect.tickUpper = BigInt.fromI32(event.tickUpper as i32);
  collect.logIndex = BigInt.fromI32(tx.logOrdinal as i32);
